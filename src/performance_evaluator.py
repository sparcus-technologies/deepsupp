...
class SupportLevelEvaluator:
    ...
    def find_price_file(self):
        ...
        possible_names = ["sp500_historial_prices.csv"]
    ...
    def load_historical_prices(self):
        ...
        try:
            ...
            print(
                f"✅ Loaded historical prices: {len(self.historical_prices.columns)} tickers"
            )
            ...
            if current_price == min(clean_prices.iloc[i - 5 : i + 6]):
                ...
                if len(future_prices) > 0:
                    ...
                    if bounce_pct > 0.01:  # At least 1% bounce
                        ...
                        # Support duration
                        hold_duration = self._calculate_support_duration(
                            clean_prices, i, current_price
                        )
                        support_durations.append(hold_duration)

                        # False breakout check
                        false_breakout = self._check_false_breakout(
    ...
    def _calculate_volume_confirmation(
        self, volume: np.ndarray, support_idx: int
    ) -> float:
    ...
    def _analyze_regime_performance(
        self, regimes: List[str], supports: List[float]
    ) -> Dict[str, float]:
    ...
    def _calculate_support_duration(
        self, prices: pd.Series, support_idx: int, support_level: float
    ) -> int:
    ...
    def _check_false_breakout(
        self, prices: pd.Series, support_idx: int, support_level: float, duration: int
    ) -> bool:
    ...
    def load_support_method_results(self) -> Dict[str, pd.DataFrame]:
        ...
        if not csv_files:
            ...
            csv_files = glob.glob(
                os.path.join(self.datasets_dir, "*_support_levels.csv")
            )
    ...
    def save_results(self, results_df: pd.DataFrame, report: str):
        ...
        results_file = os.path.join(
            self.datasets_dir, "enhanced_support_methods_evaluation.csv"
        )
